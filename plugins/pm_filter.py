...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # don't change anything without contacting me @creatorrio
        ...
        await query.message.edit_text(
            text=script.JOINUP_TXT.format(query.from_user.mention),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            m=await message.reply_sticker("CAACAgUAAxkBAAI1aWVkllJ6JttM8qxrUMdLHLoTJJD9AAKJBwACtw_AVRE1_5hD1fOSHgQ")
            search = search.lower()
            find = search.split(" ")
            search = ""
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            try:
               if settings['auto_delete']:
                    await asyncio.sleep(300)
                    m=await message.reply_sticker("CAACAgQAAxkBAAJG4WV8PrrFY-kn7r5H06TRZHtPzi-WAAL2EAACULvgU8SbYPGo146YHgQ")
                    await hmm.delete()
                    await message.delete()
            except KeyError:
                await save_group_settings(message.chat.id, 'auto_delete', True)
